...
public class ApplicationExecution extends Thread {
	 ...
	 * We use that class for the process such as getting list of terms, finding term' id and inserting any Twitter data for our tables, 
	 */
	protected static DataFormat dataFormat = new DataFormat();
	...
	public static void execution(String[] args) {
		...
		try {
			...
			/*
			 * Authenticate Twitter to access its data by using the information
			 * of consumer and accessToken as a arg parameter. Set the
			 * Configuration for twitter by using these arg parameters
			 */
			twProcess = TwitterProccess.authenticate(args);

			/*
			 * Get some details such as connection, twitter object and our term'
			 * list
			 */
			...
			/*
			 * Create the query by using terms list, then Check it weather it's
			 * empty.
			 */
			if ((queryString1 = twProcess.createQuery(0, 10))
					.isEmpty() /*
								 * ||(queryString2 =
								 * twProcess.createQuery(listSize/3+1,
								 * listSize*2/3)).isEmpty() || (queryString3 =
								 * twProcess.createQuery(listSize*2/3+1,
								 * listSize)).isEmpty()
								 */) {
			...
			/*
			 * Add the query for our Query arrays. Execute the searching.
			 */
			query[0] = new Query(
					queryString1); /*
